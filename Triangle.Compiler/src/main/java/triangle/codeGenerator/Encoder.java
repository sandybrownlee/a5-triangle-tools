...
public final class Encoder implements ActualParameterVisitor<Frame, Integer>,
	...
	public Void visitLoopWhileCommand(LoopWhileCommand ast, Frame frame) {
		var loopAddr = emitter.getNextInstrAddr();
		ast.PreCommand.visit(this, frame);

		ast.E.visit(this, frame);
		var exprAddr = emitter.emit(OpCode.JUMPIF, Machine.falseRep, Register.CB, 0);

		ast.PostCommand.visit(this, frame);
		emitter.emit(OpCode.JUMP, 0, Register.CB, loopAddr);

		emitter.patch(exprAddr);
