...
public final class Encoder implements ActualParameterVisitor<Frame, Integer>,
	...
	public Void visitLoopWhileCommand(LoopWhileCommand ast, Frame frame) {
		var startOfLoop = emitter.getNextInstrAddr(); // start of the loop
		ast.c1.visit(this, frame); //visits first command node
		ast.e.visit(this, frame); //visits expression
		var exit = emitter.emit(OpCode.JUMPIF, Machine.falseRep, Register.CB, startOfLoop); // jumps to exit if expression is false
		ast.c2.visit(this, frame);
		emitter.emit(OpCode.JUMP, startOfLoop);
		emitter.patch(exit);
