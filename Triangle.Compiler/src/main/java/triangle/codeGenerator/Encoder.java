...
import triangle.codeGenerator.Label;
...
public final class Encoder implements ActualParameterVisitor<Frame, Integer>,
    ...
    public Void visitLoopWhileCommand(LoopWhileCommand ast, Frame frame) {
        ...
        // Emit label for the start of the while part
        ...
        // Emit code for the conditional jump
        Label whileEnd = new Label();
        emit(OpCode.JUMPIF, 0, Register.CB, whileEnd.displacement);

        // Emit code for C2
        ast.C2.visit(this, frame);

        // Emit code for the unconditional jump back to the start
        emit(OpCode.JUMP, 0, Register.CB, whileStart.displacement);

        // Emit label for the end of the while part
        emitLabel(whileEnd);
    ...
    private void emitLabel(Label label) {
        label.displacement = emitter.getNextInstrAddr();
        emitter.emit(OpCode.LOAD, Register.CB, 0); // Using LOAD as a placeholder
    }

    private void emit(OpCode opCode, int length, Register register, int operand) {
        emitter.emit(opCode, length, register, operand);
    }

    @Override
    public Integer visitSquareExpression(SquareExpression ast, Frame frame) {
        var valSize = ast.type.visit(this, frame);
        ast.visit(this, frame);
        return valSize;
    }
    ...
    public static void writeTableDetails(AbstractSyntaxTree ast) {
        // Implement details if needed for debugging or table output
    }
