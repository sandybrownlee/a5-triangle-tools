...
public class ConstantFolder implements ActualParameterVisitor<Void, AbstractSyntaxTree>,
	...
	public AbstractSyntaxTree visitLoopWhileCommand(LoopWhileCommand ast, Void arg) {
		//visit first command block C1
		ast.visit(this,arg);
		//visit the condition E
		AbstractSyntaxTree replacement = ast.E.visit(this, arg);
		if (replacement != null) {
			//if the condition has been modified, replace it with the updated expression
            ast.E = (Expression) replacement;
        }
		//visit the second command block C2
