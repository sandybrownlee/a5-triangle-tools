	...
	public AbstractSyntaxTree foldBinaryExpression(AbstractSyntaxTree node1, AbstractSyntaxTree node2, Operator o) {
		...
		if ((node1 instanceof IntegerExpression) && (node2 instanceof IntegerExpression)) {
			...
			if (foldedValue instanceof Integer) {
				...
				Identifier bool_identifier = new Identifier(foldedValue.toString(), node1.getPosition());
				if (foldedValue.toString().equals("true")){
					bool_identifier.decl = StdEnvironment.trueDecl;
				}
				else if (foldedValue.toString().equals("false")){
					bool_identifier.decl = StdEnvironment.falseDecl;
				}
					SimpleVname vn = new SimpleVname(bool_identifier, node1.getPosition());
		...
		// any unhandled situation (i.e. or ve, not foldable) is ignored
		return null;
	}
