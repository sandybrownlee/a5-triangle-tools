...
public class SummaryVisitor implements ActualParameterVisitor<Void, AbstractSyntaxTree>,
    ...
    //Task 5.a counter for each instance of a Binary expression, if command and while Command
    private int binExpCount = 0;
    private int ifCommCount = 0;
    private int whileCommCount = 0;

    //Task 5.a print the summary stats
    public void printSummaryStats() {
        System.out.println("There is a total of " + binExpCount + " character expressions.");
        System.out.println("There is a total of " + ifCommCount + " if commands.");
        System.out.println("There is a total of " + whileCommCount + " while commands.");
    }
    ...
    public AbstractSyntaxTree visitBinaryExpression(BinaryExpression ast, Void arg) {
       // Task 5.a increment the BinaryExpression count
        binExpCount++;
    ...
    public AbstractSyntaxTree visitIfCommand(IfCommand ast, Void arg) {
        // Task 5.a increment the if command count
        ifCommCount++;
        ast.C1.visit(this);
        ast.C2.visit(this);
        AbstractSyntaxTree replacement = ast.E.visit(this);
    ...
    public AbstractSyntaxTree visitWhileCommand(WhileCommand ast, Void arg) {
        // Task 5.a increment the while command count
        whileCommCount++;
        ast.C.visit(this);
        AbstractSyntaxTree replacement = ast.E.visit(this);
        if (replacement != null) {
