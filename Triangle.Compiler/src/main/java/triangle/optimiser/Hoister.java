...
public class Hoister implements ActualParameterVisitor<Void, AbstractSyntaxTree>,
    ...
    /// Counter to mark if we are currently visiting while in a loop.
    /// int rather than bool so if there are nested loops we can increment on entry and decrement on exit.
    private int inLoop;

    /// Whether visitor should replace AST nodes with constant values.
    private boolean replacementPass;

    /// The expressions to be placed into temporary variables outside loop
    private List<AbstractSyntaxTree> expressions;

    /// List of Vnames that are updated in a loop, and therefor can't be hoisted.
    private List<Vname> updatedVnames;

    public Hoister() {
        updatedVnames = new ArrayList<>();
        inLoop = 0;
    ...
    public AbstractSyntaxTree visitAssignCommand(AssignCommand ast, Void arg) {
        // If in loop, add Vname to list of vars that are updated
        if (inLoop > 0 && !replacementPass) updatedVnames.add(ast.V);
