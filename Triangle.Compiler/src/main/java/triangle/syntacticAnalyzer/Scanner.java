...
public final class Scanner {
    ...
    private Token.Kind scanToken() {
        ...
        switch (currentChar) {
            ...
            case 'k':
            case 'l':
                takeIt(); // consume initial L character
				while(isLetter(currentChar) || isDigit(currentChar)){ //continue consuming characters as long as they are letters
					takeIt();
				}
                //check id the accumulated spelling matches keyword "loop"
				if(currentSpelling.toString().equals("loop")){
					return Token.Kind.LOOP; // Return LOOP if 'loop' is identified
				}else{
					return Token.Kind.IDENTIFIER; //otherwise return general identifier
				}
