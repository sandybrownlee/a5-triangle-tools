        ...
        switch (currentToken.kind) {
            ...
            case IDENTIFIER: {
                ...
                if (currentToken.kind == Token.Kind.LPAREN) {
                    ...
                    // translate a** to a = a*a
                    if (currentToken.kind == Token.Kind.OPERATOR && currentToken.spelling.equals("**")) {
                        acceptIt();

                        // this wraps the variable name in an VnameExpression
                        VnameExpression vne = new VnameExpression(vAST, commandPos);

                        // the operator is a * for squaring
                        Operator op = new Operator("*", commandPos);

                        // now we assemble the expression into binary for a*a
                        Expression eAST = new BinaryExpression(vne, op, vne, commandPos);

                        // set last line of command for debugging
                        finish(commandPos);

                        // make thee assignment with binary expression on right
